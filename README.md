...
Para evitar usar `sudo` con Docker en Linux:
```bash
...
```mermaid
flowchart TD
A[Levantar Docker] --> B[Ir a 127.0.0.1:8000/docs]
B --> C[Verificar /api/professor/status]
C --> D[Ejecutar /api/professor/scrape]
D --> E[Ejecutar parse_scraped_data.py]
E --> F[Almacenar con /api/doc_advisor]
